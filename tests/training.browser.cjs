...
(async () => {
 ...
 assert.equal(await page.locator('#leaderboardTitle').textContent(), '瞄准命中榜');
 assert.match(await page.locator('#leaderboardState').textContent(), /服务暂时不可用/);
 await page.keyboard.press('Escape');
 await page.locator('#gamePickerButton').click(); await page.locator('.game-card[data-game="reaction"]').click();
 assert.deepEqual(errors, []);
 console.log('PASS: 5-round reaction average/best, false start, keyboard, interrupted wait, aim hits/misses/right-click/deadline/restart, metric leaderboard, touch, mobile and no errors.');
 await browser.close();
